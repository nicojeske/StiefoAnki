...
def generateSvgPath(curves: List[Curve], offset: tuple[float, float]) -> Generator[str, Any, None]:
    for curve in curves:
        ...
        yield f"<path d=\"M {curve.start[0]} {curve.start[1]} C {curve.control1[0]} {curve.control1[1]}, {curve.control2[0]} {curve.control2[1]}, {curve.end[0]} {curve.end[1]}\" class=\"b\"/>"
...
def generateSvg(wordList: List[Word]) -> str:
    ...
    svg = f"<svg width=\"{width}\" height=\"{height}\"><style>.b {{fill: none; stroke: black; stroke-width: 2; }}</style>{lines}{svg}</svg>"
